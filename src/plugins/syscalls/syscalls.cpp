    ...
    const std::vector<uint64_t>& args, bool is_ret, bool ret_success)
{
    for (size_t i = 0; i < args.size(); ++i)
    {
        ...
        {
            original_args.emplace(std::string(original_arg.name), fmt::Estr("<out>"));
            continue;
        }
        ...
        if (this->dereference_args)
        {
            PRINT_DEBUG("Arg '%s': raw_value=0x%lx, is_ptr=%d, ptr_for_type=%d, is_complete=%d\n",
                original_arg.name,
                raw_value,
                type_info.is_ptr,
                type_info.ptr_for_type,
                is_value_complete);
        }
        ...
        original_args.emplace(std::string(original_arg.name), fmt::Xval(raw_value));
        ...
        {
            extra_args.emplace(std::string("*") + std::string(original_arg.name), fmt::Xval(*dereferenced_value));
        }
    }
}
